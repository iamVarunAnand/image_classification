...
class XResNeXt:
    ...
    @staticmethod
    def residual_module(data, K, stride, chan_dim, groups, red = False, reg = 1e-4, bn_eps = 2e-5, bn_mom = 0.9,
                        name = "res_block"):
        # shortcut branch
        shortcut = data
        ...
        conv1 = Conv2D(int(K * 0.5), (1, 1), use_bias = False, kernel_regularizer = l2(reg),
        ...
        conv2 = Conv2D(int(K * 0.5), (3, 3), strides = stride, groups = groups, padding = "same", use_bias = False,
    ...
    @staticmethod
    def build(height, width, depth, classes, stages, filters, groups, stem_type = "imagenet",
        ...
        for i in range(0, len(stages)):
            ...
            x = XResNeXt.residual_module(x, filters[i + 1], stride, chan_dim, groups = groups, reg = reg, red = True,
            ...
            for j in range(0, stages[i] - 1):
                ...
                x = XResNeXt.residual_module(x, filters[i + 1], (1, 1), chan_dim, groups = groups, reg = reg,
                                             bn_eps = bn_eps, bn_mom = bn_mom, name = name)
